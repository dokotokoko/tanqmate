...
**表示要素**
- 認証方法変更と再登録が必要であることを伝える一時ニュース
- メールアドレス入力
- パスワード入力
- ログインボタン（主文言は「AIとの探究を始める」）
- Googleでログインボタン
- 「新規登録」リンク → `/signup`
- 「パスワードをお忘れの方はこちら」リンク → `/password-reset`
- 先生・管理者向けログイン導線 → `/teacher/signin`

**UI**
- PCでも縦長のスマホ風パネルを主操作面とする
- 一時ニュースはフォーム操作前に読める位置に表示する
- ログイン失敗時にも一時ニュースは消えない
...
**同意概要**
- 正式な内容はプライバシーポリシー本文を正本とする
- 画面上では、生徒が知っておくべき以下の要点を短く表示する
  - 興味、テーマ、問いなどはAI支援に使われる
  - 先生に共有される内容は支援に必要な最小限を基本とする
  - raw対話ログや入力途中の内省は不用意に先生へ見せない
  - 研究・改善目的で使う場合は個人が特定されない形にする
...
- 利用規約とプライバシーポリシーの両方に同意するまで登録できない

**UI**
- PCでも縦長のスマホ風パネルを主操作面とする
- 同意概要はPCでは余白、モバイルではフォーム下に表示する
